...
[[noreturn]] void *Salesman(void *param) {
    ...
    while (true) {
        sem_wait(&needed[num]); // Ожидание пока покупатель не вызовет продавца
...
void *Buyer(void *param) {
    ...
    for (int i = 0; i < n; ++i) {
        ...
        sem_wait(&available[k]); // Ожидание когда осовободиться продавец
...
int main(int argc, char *argv[]) {
    ...
    for (int i = 0; i < 3; ++i) {
        sem_init(&needed[i], 0, 0); // Иницализация семафора для вызова i - го продавца
        sem_init(&available[i], 0, 1);  // Иницализация семафора обозначающего что i - ый продавец свободен
    }
